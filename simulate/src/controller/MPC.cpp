...
{
    uMin.resize(5*rSize);
    uMax.resize(5*rSize); // for friction cone constraint
    xMin.resize(15);
    xMax.resize(15);
       // input inequality constraints
    for(int i = 0; i<5*rSize;i++)
    {
        uMin(i) = 0;
        uMax(i) = OsqpEigen::INFTY;    
    }
    ...
    xMax << OsqpEigen::INFTY, OsqpEigen::INFTY, OsqpEigen::INFTY, OsqpEigen::INFTY, OsqpEigen::INFTY, OsqpEigen::INFTY, OsqpEigen::INFTY,
            OsqpEigen::INFTY, OsqpEigen::INFTY, OsqpEigen::INFTY, OsqpEigen::INFTY, OsqpEigen::INFTY, 0, 0, -9.81*m;
...
{
    Eigen::MatrixXd temp = Eigen::MatrixXd::Zero(15 * (mpcWindow + 1) + 15 * (mpcWindow + 1) + 5*rSize * mpcWindow,
                            15 * (mpcWindow + 1) + 3*rSize * mpcWindow);
    ...
    for(int i = 0;i<mpcWindow;i++)
    {
        temp.block(2*15*(mpcWindow + 1)+5*i,15*(mpcWindow +1)+3*i,5,3) = FrConstrainMatrix;
    }
                                  ...
                                  Eigen::VectorXd& upperBound)
{
    ...
    Eigen::VectorXd lowerInequality
        = Eigen::MatrixXd::Zero(15 * (mpcWindow + 1) + 5*rSize * mpcWindow, 1);
    ...
    for (int i = 0; i < mpcWindow; i++)
    {
        lowerInequality.block(5*rSize * i + 15 * (mpcWindow + 1), 0, 5*rSize, 1) = uMin;
        upperInequality.block(5*rSize * i + 15 * (mpcWindow + 1), 0, 5*rSize, 1) = uMax;
    }
    ...
    lowerBound = Eigen::MatrixXd::Zero(2 * 15 * (mpcWindow + 1) + 5*rSize * mpcWindow, 1);
    lowerBound << lowerEquality, lowerInequality;

    upperBound = Eigen::MatrixXd::Zero(2 * 15 * (mpcWindow + 1) + 5*rSize * mpcWindow, 1);
...
bool MPC::init(Eigen::VectorXd& x_init,Eigen::VectorXd& x_ref,double delT, Eigen::Matrix3d& Rz, Eigen::Matrix3d& gI, std::vector<Eigen::VectorXd>& r,double m,double fc,OsqpEigen::Solver& solver)
{   
    mpcWindow = 10;
    int rSize = r.size();
    this -> rSize = r.size();
    ...
    setInequalityConstraints(xMax, xMin, uMax, uMin,m,rSize);
    ...
    castMPCToQPConstraintMatrix(a, b, mpcWindow, rSize, fc,linearMatrix);
    ...
    solver.data()->setNumberOfConstraints(2 * 15 * (mpcWindow + 1) + 5*rSize * mpcWindow);
